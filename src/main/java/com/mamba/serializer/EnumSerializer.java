...
public class EnumSerializer extends JsonSerializer<Enum<? extends EnumDefinition>> implements ContextualSerializer {
    ...
    @Override
    public void serialize(Enum<? extends EnumDefinition> value, JsonGenerator gen,
        ...
        if (!this.beanProperty.getType().isEnumType()) {
            log.error("当前序列化对象不是枚举: {}:{}", beanProperty.getType().getRawClass().getSimpleName(),
                beanProperty.getFullName().getSimpleName());
            gen.writeObject(null);
            return;
        }
        if (!EnumDefinition.class.isAssignableFrom(this.beanProperty.getType().getRawClass())) {
            log.error("当前序列化对象未实现EnumComment: {}:{}", beanProperty.getType().getRawClass().getSimpleName(),
                beanProperty.getFullName().getSimpleName());
            gen.writeObject(null);
            return;
        }
